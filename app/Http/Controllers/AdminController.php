...
class AdminController extends Controller
{
    public function admin()
    {
    	...
    	public function remove_top(Request $request)
		{
			//dd($request->get('id'));
			//$userid = Auth::user()->id;
   ...
   public function adminlogin(Request $request)
    ...
    public function dashboard()
    {
  		...
  		public function hdn()
        {
        	$result = DB::table('headlines')->select('headlines.*','boi_news.newsid','boi_news.newstitle')
						...
						->take(20)
						->get();
            return view('backend/admin_addnewheadline')->with('data', $result);
        }
		...
		public function submit_headline(Request $request)
	       ...
	       // return redirect()->back();
	    	return redirect::to('admin/headline')->with('success', 'Successfully Added.');	
   		}
		...
		public function update_headline(Request $request)
		{
			...
			return redirect::to('admin/headline');
   		...
   		public function headline_delete($id)
		{
			 ...
			 return redirect::to('/admin/headline')->with('delete', 'Successfully Delete.');	
		}
		...
		public function headline_edit($id)
		{
			...
			return view('backend.adminedit_headline')->with('edit', $edit);		
		}
    ...
    //============================== POSTS MANAGE ===========================
		...
		public function submit_post(Request $request)
		{
			...
			return redirect::to('/admin/posts')->with('success', 'Successfully Added.');	
		}
		...
		public function update_post(Request $request)
		{
			...
			{
				$serials = 	DB::table('boi_news')->where('serial', $request->get('serial'))->get();
				foreach ($serials as $key => $value) 
				{
					DB::table('boi_news')->where('serial', $request->get('serial'))
					->update(
						array('serial' => null)
					);	
				}
		   ...
		   if (!empty($file)) 
		   {
					...
					return redirect::to('/superadmin/posts')->with('success', 'Successfully Update.');
			}
			else
			{
				return redirect::to('/superadmin/posts')->with('success', 'Successfully Update.');
...
*/
        ...
        public function user_posts()
        {
			...
			if(Auth::user()->type==1)// 1 for editor and 2 for reporter
			{
		...
		public function user_submit_post(Request $request,LaravelFacebookSdk $fb)
		{
           ...
           // dd("i am here ....");
	    	...
	    	$table->newsupdateby = $request->get('newsupdateby');

	    	//dd($table->newsupdateby);
		...
		//facebook auto post share end
			...
			$categories_data=$request->get('categories');
			if($categories_data)
			foreach($categories_data as $menus)
			{
				if($name = explode("_",$menus))
				{
					$mid = $name[0];
			...
			{
				return redirect::to('/admin/user_post_edit/'.$last_newid)->with('success', 'Successfully Added.');	 	
			}
...
//if($request->get('top_news_main')!=$request->get('top_news'))
		...
		if($request->get('top_news_main') != $request->get('top_news'))
		{
			...
			array(
					...
					'newstitle' 			=> $request->get('title'),
					'newsupdateby' 			=> $request->get('newsupdateby'),
		   ...
		   if (!empty($file)) 
		   {
					...
					return redirect::to('/admin/user_post_edit/'.$last_newid)->with('success', 'Successfully Update.');
...
public function ads()
{
	$result = DB::table('boi_gallery')->select('*')
	->orderBy('id', 'DESC')->get();
	return view('backend/ads_page')->with('data', $result);;
}

public function newaddads()
{
	return view('backend/addads_page');
}
...
public function submit_ads(Request $request)
{
	...
	return redirect::to('/admin/ads')->with('success', 'Successfully Added.');	

}

public function ads_edit($id)
{
	$edit = DB::table('boi_gallery')->select('*')
	->where('id',$id)->first();
	
	return view('backend/editbanner_page_01', compact('edit')); 
}
...
public function update_ads(Request $request)
{
	   ...
	   if ($input) 
	   {
		   ...
		   if (!empty($file)) 
		   {
			   ...
			   if ($validator->fails()) 
			   {
				   return redirect::to('admin/editads/' . $request->get('id'))->withErrors($validator);
			   }
			   else
			   {
					$image              = $request->file('image');
					...
					return redirect::to('/admin/ads')->with('success', 'Update Successfully Done.');   
			   }
			}
			else
			{
...
*/
	...
	{
		$result = DB::table('boi_gallery')->select('boi_gallery.*','boi_galleryalbum.id AS bgalbumid','boi_galleryalbum.atitle')
	...
	public function newaddgallery()
	{
		...
		return view('backend/addbanner_page_01', compact('galleryalbum'));
	}
   	...
   	public function submit_gallery(Request $request)
   	{
    	...
    	return redirect::to('/admin/gallery')->with('success', 'Successfully Added.');	

   	}

   	public function gallery_edit($id)
	{
		$edit = DB::table('boi_gallery')->select('boi_gallery.*','boi_galleryalbum.id AS bgalbumid','boi_galleryalbum.atitle')
		->leftJoin('boi_galleryalbum','boi_galleryalbum.id','=','boi_gallery.type')
		->where('boi_gallery.id',$id)->first();

		$galleryalbum = DB::table('boi_galleryalbum')->orderBy('id','DESC')->get();
		
		return view('backend/editbanner_page_01', compact('edit','galleryalbum')); 
	}
   	...
   	public function update_gallery(Request $request)
   	{
           ...
           if ($input) 
           {
               ...
               if (!empty($file)) 
               {
                   ...
                   if ($validator->fails()) 
                   {
                       return redirect::to('admin/editbanner/' . $request->get('id'))->withErrors($validator);
                   }
                   else
                   {
                        $image              = $request->file('image');
                        ...
                        return redirect::to('/admin/gallery')->with('success', 'Update Successfully Done.');   
                   }
                }
                else
                {
...
*/	   
	...
	{
		return view('backend/addgalbum_page_01');
	}
   	...
   	public function submit_galleryalbum(Request $request)
   	{
    	...
    	return redirect::to('/admin/galleryalbum')->with('success', 'Successfully Added.');	
   	...
   	public function galleryalbum_edit($id)
	{
		...
		return view('backend/editgalbum_page_01')->with('edit', $result);; 
	}
   	...
   	public function update_galleryalbum(Request $request)
   	{
			...
			return redirect::to('/admin/galleryalbum')->with('success', 'Update Successfully Done.');   
   	}
...
*/
	...
	{
		return view('backend/addvote_page_01');
	}
   	...
   	public function submit_vote(Request $request)
   	{
    	...
    	return redirect::to('/admin/vote')->with('success', 'Successfully Added.');	
   	...
   	public function vote_edit($id)
	{
		...
		return view('backend/editvote_page_01')->with('edit', $result); 
	}
   	...
   	public function update_vote(Request $request)
   	{
           $input = $request->except('_token');
			...
			return redirect::to('/admin/vote')->with('success', 'Update Successfully Done.');   
...
*/
	...
	{
        return view('backend/add_media_page_01');
	}
	...
	protected function submit_media(Request $request)
	{
		...
		return redirect::to('/admin/media')->with('success', 'Successfully Added.');	
	}
	...
	protected function update_media(Request $request)
	{
		...
		return redirect::to('/admin/media')->with('success', 'Successfully Update.');	
	}
	...
	public function media_edit($id)
	{
		...
		return view('backend/edit_media_page_01' , compact('edit'));		
	}
	...
	{	
		 ...
		 return redirect::to('/admin/media')->with('success', 'Successfully Delete.');	
	}
